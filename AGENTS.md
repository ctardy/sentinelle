...
- **Langues supportées par l'app** : **application internationale** — FR (défaut / source de vérité), EN, ES, DE à terme, extensible. Aucune chaîne visible par l'utilisateur en dur dans le code : tout passe par les ressources localisées Android (`res/values/`, `res/values-en/`, `res/values-es/`, `res/values-de/`) et par les fichiers JSON localisés de la KB. Découper `strings_*.xml` par domaine (`strings.xml`, `strings_audit.xml`, `strings_onboarding.xml`, `strings_a11y.xml`, etc.)
